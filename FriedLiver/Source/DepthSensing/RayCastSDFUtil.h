...
struct RayCastData {
	...
	__device__
	bool trilinearInterpolationSimpleFastFast(const HashDataStruct& hash, const float3& pos, float& dist, float3& color) const {
		...
		Voxel v = hash.getVoxel(posDual+make_float3(0.0f, 0.0f, 0.0f)); if(v.weight == 0) return false; float3 vColor = make_float3(v.color.x, v.color.y, v.color.z); dist+= (1.0f-weight.x)*(1.0f-weight.y)*(1.0f-weight.z)*v.sdf; colorFloat+= (1.0f-weight.x)*(1.0f-weight.y)*(1.0f-weight.z)*vColor;
			  ...
			  //printf("%f\n", v.color.y);
		color = make_float3(colorFloat.x, colorFloat.y, colorFloat.z);//v.color;
		
		return true;
	}
	//__device__
	//bool trilinearInterpolationSimpleFastFast(const HashData& hash, const float3& pos, float& dist, float3& color) const {
	//	const float oSet = c_hashParams.m_virtualVoxelSize;
	//	const float3 posDual = pos-make_float3(oSet/2.0f, oSet/2.0f, oSet/2.0f);
	//	float3 weight = frac(hash.worldToVirtualVoxelPosFloat(pos));
	...
	// d0 near, d1 far
	__device__
		bool findIntersectionBisection(const HashDataStruct& hash, const float3& worldCamPos, const float3& worldDir, float d0, float r0, float d1, float r1, float& alpha, float3& color) const
	{
...
#pragma unroll 1
	...
	float3 gradientForPoint(const HashDataStruct& hash, const float3& pos) const
	{
		...
		float distp00; float3 colorp00; trilinearInterpolationSimpleFastFast(hash, pos-make_float3(0.5f*offset.x, 0.0f, 0.0f), distp00, colorp00);
		float dist0p0; float3 color0p0; trilinearInterpolationSimpleFastFast(hash, pos-make_float3(0.0f, 0.5f*offset.y, 0.0f), dist0p0, color0p0);
		float dist00p; float3 color00p; trilinearInterpolationSimpleFastFast(hash, pos-make_float3(0.0f, 0.0f, 0.5f*offset.z), dist00p, color00p);

		float dist100; float3 color100; trilinearInterpolationSimpleFastFast(hash, pos+make_float3(0.5f*offset.x, 0.0f, 0.0f), dist100, color100);
		float dist010; float3 color010; trilinearInterpolationSimpleFastFast(hash, pos+make_float3(0.0f, 0.5f*offset.y, 0.0f), dist010, color010);
		float dist001; float3 color001; trilinearInterpolationSimpleFastFast(hash, pos+make_float3(0.0f, 0.0f, 0.5f*offset.z), dist001, color001);
				...
				//if(lastSample.weight > 0 && ((lastSample.sdf > 0.0f && dist < 0.0f) || (lastSample.sdf < 0.0f && dist > 0.0f))) //hack for top down video
				{
					...
					if(b && abs(lastSample.sdf - dist) < rayCastParams.m_thresSampleDist)
					{
						if(abs(dist) < rayCastParams.m_thresDist)
						{
							...
							d_colors[dTid.y*rayCastParams.m_width+dTid.x] = make_float4(color2.x, color2.y, color2.z, 1.0f);
