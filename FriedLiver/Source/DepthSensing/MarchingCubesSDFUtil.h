...
//#endif
	void extractIsoSurfaceAtPosition(const float3& worldPos, const HashDataStruct& hashData, const RayCastData& rayCastData)
	{
		...
		float3 p000 = worldPos+make_float3(M, M, M); float dist000; float3 color000; bool valid000 = rayCastData.trilinearInterpolationSimpleFastFast(hashData, p000, dist000, color000);
		float3 p100 = worldPos+make_float3(P, M, M); float dist100; float3 color100; bool valid100 = rayCastData.trilinearInterpolationSimpleFastFast(hashData, p100, dist100, color100);
		float3 p010 = worldPos+make_float3(M, P, M); float dist010; float3 color010; bool valid010 = rayCastData.trilinearInterpolationSimpleFastFast(hashData, p010, dist010, color010);
		float3 p001 = worldPos+make_float3(M, M, P); float dist001; float3 color001; bool valid001 = rayCastData.trilinearInterpolationSimpleFastFast(hashData, p001, dist001, color001);
		float3 p110 = worldPos+make_float3(P, P, M); float dist110; float3 color110; bool valid110 = rayCastData.trilinearInterpolationSimpleFastFast(hashData, p110, dist110, color110);
		float3 p011 = worldPos+make_float3(M, P, P); float dist011; float3 color011; bool valid011 = rayCastData.trilinearInterpolationSimpleFastFast(hashData, p011, dist011, color011);
		float3 p101 = worldPos+make_float3(P, M, P); float dist101; float3 color101; bool valid101 = rayCastData.trilinearInterpolationSimpleFastFast(hashData, p101, dist101, color101);
		float3 p111 = worldPos+make_float3(P, P, P); float dist111; float3 color111; bool valid111 = rayCastData.trilinearInterpolationSimpleFastFast(hashData, p111, dist111, color111);
	...
	{
		Vertex r1; r1.p = p1; r1.c = make_float3(c1.x, c1.y, c1.z);
		Vertex r2; r2.p = p2; r2.c = make_float3(c2.x, c2.y, c2.z);
		...
		res.c.x = (float)(c1.x + mu * (float)(c2.x - c1.x)); // Color
		res.c.y = (float)(c1.y + mu * (float)(c2.y - c1.y));
		res.c.z = (float)(c1.z + mu * (float)(c2.z - c1.z));
